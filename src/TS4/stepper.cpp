...
namespace TS4
{
    ...
    {
        StepperBase::doRotateAsync(v == 0 ? vMax : v, acc);
    }
    ...
    {
        StepperBase::doMoveAsync(pos + delta, 200, 0, (v == 0 ? std::abs(vMax) : v), acc);
    }

    void Stepper::stopAsync()
    {
        StepperBase::doStopAsync(0, acc);
    }

    void Stepper::moveAsync()
    {
        StepperBase::doMoveAsync(target,200, 0, std::abs(vMax), acc);
    }
